...
public interface EmotionalAnnotationRepository extends CrudRepository<EmotionalAnnotationEntity, Integer> {
    ...
    @Query(value = "SELECT CONCAT(ea.id,','" +
            ...
            ",IF(ea.example1 IS NULL,'null',CONCAT('\"',REPLACE(ea.example1,'\"','####'),'\"')),','" +
            ",IF(ea.example2 IS NULL,'null',CONCAT('\"',REPLACE(ea.example2,'\"','####'),'\"'))) " +
            " FROM emotional_annotations ea WHERE ea.sense_id IN (:sense_ids)", nativeQuery = true)
    public List<String> findAllForSenseIdsAndParseString(@Param("sense_ids") Integer[] sense_ids);
    ...
    @Query(value = "SELECT CONCAT(ea.id,','" +
            ...
            ",IF(ea.example2 IS NULL,'null',CONCAT('\"',REPLACE(ea.example2,'\"','####'),'\"'))) " +
            "FROM emotional_annotations ea" +
            " WHERE ea.sense_id>=:begin AND ea.sense_id<:end AND ea.sense_id IN (:sense_ids)", nativeQuery = true)
    public List<String> findAllForSenseIdsAndParseStringBatch(@Param("sense_ids") Integer[] sense_ids,
