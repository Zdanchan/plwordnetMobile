...
public interface EmotionalAnnotationRepository extends CrudRepository<EmotionalAnnotationEntity, Integer> {
    @Query(value = "SELECT CONCAT(ea.id,',',ea.sense_id,',',ea.has_emotional_characteristic,',',ea.super_anotation,',','\"',ea.emotions,'\"',',''\"',ea.valuations,'\"',',''\"',ea.markedness,'\"',',''\"',ea.example1,'\"',',''\"',ea.example2,'\"') FROM emotional_annotations ea", nativeQuery = true)
